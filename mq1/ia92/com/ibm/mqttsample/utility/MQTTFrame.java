//-----------------------------------------------------------------------------
// Source File Name: MQTTFrame.java
//
...
// IBM Corp.
//
// Version $Id: MQTTFrame.java,v 1.10 2005/12/07 13:24:11 iharwood Exp $
//
//-----------------------------------------------------------------------------
...
import com.ibm.mqttsample.persistence.MqttFilePersistence;
 ...
 */
public class MQTTFrame implements ActionListener, MqttSimpleCallback, Runnable {
    ...
    private final static String PROP_FILE = "wmqtt.properties";
		...
		theFrame.addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
                ...
                try {
                    ...
                    // Now write to disk
                    props.store( propFile, "WMQTT Utility properties" );
	...
	protected void init( Container contentPane ) {
		...
		wmqttComp.add( pubsub, BorderLayout.CENTER );
        ...
        tabbedGui.addTab( "WMQTT", wmqttComp );
    ...
    public void connect( String connStr, boolean usePersistence ) throws MqttException {
		...
		if ( mqtt == null ) {
			...
			if ( usePersistence ) {
				persistence = new MqttFilePersistence( optionsComp.getPersistenceDirectory() );
			}	
        ...
        if ( optionsComp.isLWTTopicSet() ) {
        	// If an LWT Topic is set then do the LWT connect
        	mqtt.connect(optionsComp.getClientID(), optionsComp.isCleanSessionSelected(), optionsComp.getKeepAlive(),
        	                       optionsComp.getLWTTopic(), optionsComp.getLWTQoS(), optionsComp.getLWTData(),
    ...
    public void run() {
		...
		String connStr = "";
			...
			if ( ipAddr.indexOf("://") < 0 ) {
    			connStr = MqttClient.TCP_ID + ipAddr + ":" + portNum;
   	 		} else {
   	 			connStr = ipAddr;
   	 		}
     ...
     * <LI>Disconnect - Check the client is connected and then disconnect the WMQTT protocol
     ...
     */
   	...
   	public synchronized void setTitleText( String extraText ) {
   		if ( extraText.equals("") ) {
     		frame.setTitle( "WMQTT Utility" );
   		} else {
    		frame.setTitle( "WMQTT Utility - " + extraText );
   		}		
   	}	
    		...
    		while ( (rc == -1) &&  connected ) {
        	    ...
        	    synchronized(this) { // Grab the log synchronisation lock
        	    	if ( connected ) {
        	    		...
        	    		try {
        	    			connect( mqtt.getConnection(), (mqtt.getPersistence() != null) );
        	    			rc = 0;
    ...
    public void publishArrived( String topic, byte[] data, int QoS, boolean retained ) {
    	subPanelContr.updateReceivedData( topic, data, QoS, retained );
    }	
